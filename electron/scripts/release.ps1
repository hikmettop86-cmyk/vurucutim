...
if ($NewVersion -notmatch "^\d+\.\d+\.\d+$") {
    Fail "Versiyon format hatalı: '$NewVersion' (N.N.N olmalı, örn 0.1.14)"
}

# Working tree dirty check (untracked dosyalar OK)
$dirty = git status --porcelain 2>&1 | Where-Object { $_ -notmatch "^\?\?" -and $_ -ne "" }
if ($dirty) {
    Write-Host "Git working tree kirli (commit edilmemiş değişiklik var):" -ForegroundColor Yellow
    $dirty | ForEach-Object { Write-Host "  $_" }
    Fail "Önce git commit yapıp tekrar dene."
}

# Tag exists check
$existingTag = git tag --list "v$NewVersion"
if ($existingTag) { Fail "Tag 'v$NewVersion' zaten var. Farklı versiyon seç." }

# gh auth check
$ghStatus = gh auth status 2>&1
if ($LASTEXITCODE -ne 0) { Fail "gh CLI auth değil. 'gh auth login' çalıştır." }

if ($DryRun) {
    Write-Host ""
    Write-Host "DRY-RUN: v$NewVersion için release atılacaktı, hiçbir değişiklik yapılmadı." -ForegroundColor Yellow
    return
}
...
# 1. Version bump
# ---------------------------------------------------------------------------
Step 1 7 "electron/package.json versiyonu güncelle"
...
if ($LASTEXITCODE -ne 0) { Pop-Location; Fail "sync-version başarısız" }
Pop-Location

# ---------------------------------------------------------------------------
# 3. Build
# ---------------------------------------------------------------------------
Step 3 7 "electron-builder ile setup üret (~2 dk)"
Push-Location (Join-Path $root "electron")
npm run build 2>&1 | Out-Host
if ($LASTEXITCODE -ne 0) { Pop-Location; Fail "Build başarısız" }
Pop-Location
...
foreach ($p in @($exe, $blockmap, $latestYml)) {
    if (-not (Test-Path $p)) { Fail "Build artifact bulunamadı: $p" }
}
...
if ($LASTEXITCODE -ne 0) { Fail "tag push başarısız" }

# ---------------------------------------------------------------------------
# 6. gh release create
# ---------------------------------------------------------------------------
Step 6 7 "GitHub release oluştur"
...
$releaseId = gh api "repos/$repo/releases" `
    ...
    --jq ".id" 2>&1
if ($LASTEXITCODE -ne 0) { Fail "gh release create başarısız: $releaseId" }
Write-Host "  -> release id: $releaseId"

# ---------------------------------------------------------------------------
# 7. Upload assets
# ---------------------------------------------------------------------------
Step 7 7 "asset'leri yükle ($exeSize MB)"
gh release upload "v$NewVersion" $exe $blockmap $latestYml --repo $repo 2>&1 | Out-Host
if ($LASTEXITCODE -ne 0) { Fail "asset upload başarısız" }

# ---------------------------------------------------------------------------
# Done
# ---------------------------------------------------------------------------
Write-Host ""
Write-Host "✓ v$NewVersion yayınlandı" -ForegroundColor Green
Write-Host "  https://github.com/$repo/releases/tag/v$NewVersion" -ForegroundColor Green
Write-Host ""
Write-Host "Açık VurucuTim'ler ~30 sn içinde otomatik güncelleme dialogu görür." -ForegroundColor Gray
