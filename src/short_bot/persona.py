...
def persona_block(persona: Persona) -> str:
    ...
    return (
        ...
        "SAHNEYİ CANLI TUTAN ARAÇLAR (ZORUNLU — eksikse sahne düz belgesele döner):\n"
        "  • ŞİMDİKİ ZAMAN, CANLI: 'fırlıyor', 'çakıyor', 'yere seriliyor', 'kalkıyor'.\n"
        "    Olay ŞU AN oluyor. Geçmiş zaman ('durdurdu', 'yaptı') ve bilgi tonu\n"
        "    ('bilinir ki', 'aslında') sahneyi öldürür.\n"
        "  • 'SANIRSIN X' MOTORU (kanalın EN GÜÇLÜ silahı, en çok alıntılanan unsuru):\n"
        "    hayvanın her hareketini absürt ama HİPER-SOMUT bir benzetmeye çevir —\n"
        "    neredeyse HER 2 CÜMLEDE BİR. Genel benzetme ('çorba gibi') ZAYIFtır;\n"
        "    spesifik/yerel benzetme GÜÇLÜdür: 'sanırsın modifiyeli Tofaş Doğan SLX',\n"
        "    'sanırsın yapılandırması bozulan esnaf kredisi', 'sanırsın pavyon ışığı\n"
        "    yanıp sönüyor', 'sanırsın uçak gemisinden mancınıkla fırlatıldı'. Türk\n"
        "    gündelik/esnaf/sanayi/otomotiv dünyasından çek — soyut değil, SOMUT ve tanıdık.\n"
        "  • DİYALOG / ATIŞMA: hayvanlara AĞIZ ver. İç konuşma + karşı karakterle laf\n"
        "    dalaşı: 'Kardeşim randevun yoksa geç arkada bekle', 'Sen hâlâ burada mısın\n"
        "    sinsi hortum?'. En az bir-iki replik olsun (TEK tırnak — çift tırnak JSON'u bozar).\n"
        "  • Gerçek bilgi sahnenin İÇİNE gömülür (komodo gerçekten dışkıya bulanır) —\n"
        "    ama bir DERS gibi değil, olayın bir ANI gibi anlatılır.\n\n"
