...
def compose_video_from_silent_video(
    ...
    *,
    ...
    """Compose final video using a pre-rendered silent MP4 (Remotion output)
    as the foreground stream — same audio/bg_video pipeline as compose_video
    but skips the PNG frame stage.
    """
    ...
    if bg_video_path is None:
        cmd, video_map = _build_silent_legacy_cmd(
            silent_video_path, music_path, ffmpeg_path,
            sfx_overlays, music_volume,
        )
    else:
        cmd, video_map = _build_silent_bg_video_cmd(
            silent_video_path, music_path, Path(bg_video_path),
...
def _build_silent_legacy_cmd(silent_video_path, music_path, ffmpeg_path,
                             sfx_overlays, music_volume) -> tuple[list[str], str]:
    """Silent-video + audio pipeline (no bg video).
    Inputs: 0=silent video, 1=music, 2+=optional SFX.
    """
    cmd = [
        ffmpeg_path, "-y",
...
def _build_silent_bg_video_cmd(
    silent_video_path, music_path, bg_video_path, *,
    ffmpeg_path, sfx_overlays, music_volume,
    bg_blur_px, bg_dim, fg_scale,
) -> tuple[list[str], str]:
    """Silent video over background video pipeline.
    Inputs: 0=bg video (looped), 1=silent fg video, 2=music, 3+=optional SFX.
    """
    cmd = [
        ffmpeg_path, "-y",
        "-stream_loop", "-1",
