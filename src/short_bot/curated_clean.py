...
# ── KARANLIK KLİP AYDINLATMA (retention) ─────────────────────────────────────
# İç mekân footage (kürate kliplerin çoğu) sık dim/gölgeli olur → izleyici kaçar.
# Çözüm: gölge/orta-ton kaldıran gamma (parlakları PATLATMAZ; içeride kendiliğinden
# adapte olur — karanlık banyoyu aydınlık duvardan daha çok kaldırır) + hafif doygunluk.
# Zaten iyi-aydınlık klip → dokunma (gamma yıkmasın). Kullanıcı: "klip karanlık".
_LUMA_WELL_LIT = 150.0   # ortalama parlaklık (YAVG 0-255) bunun üstündeyse dokunma


def _mean_luma(clip, ffmpeg_path: str = "ffmpeg") -> float:
    """Klibin ortalama parlaklığı (YAVG 0-255). 2 fps örnekleme → 1x1 gri piksel →
    bayt ortalaması. Ölçülemezse -1 (dokunma sinyali)."""
    try:
        out = subprocess.run(
            [ffmpeg_path, "-v", "error", "-i", str(clip),
             "-vf", "fps=2,scale=1:1,format=gray", "-f", "rawvideo", "-"],
...
def brighten_if_dark(clip, *, ffmpeg_path: str = "ffmpeg", out_path):
    """Karanlık/dim klibi gölge-kaldıran gamma + hafif doygunlukla nazikçe aydınlat.
    Aydınlatma GÜCÜ klip ne kadar karanlıksa o kadar artar (gamma 1.20→1.45). İyi-aydınlık
    klip (YAVG≥150) ya da ölçülemeyen → None (dokunma). Döner: aydınlatılmış yol ya da None."""
    yavg = _mean_luma(clip, ffmpeg_path)
    if yavg < 0 or yavg >= _LUMA_WELL_LIT:
        return None
    # Karanlıkla ölçekli gamma (gölge/orta-tonu kaldırır, parlakları sıkıştırır).
    gamma = round(min(1.45, 1.20 + (_LUMA_WELL_LIT - yavg) / 100.0 * 0.35), 3)
    out_path = Path(out_path)
    try:
        subprocess.run(
            [ffmpeg_path, "-v", "error", "-y", "-i", str(clip),
             "-vf", f"eq=gamma={gamma}:saturation=1.12:contrast=1.04",
             "-an", "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p",
             str(out_path)],
            capture_output=True, timeout=180)
        if out_path.exists() and out_path.stat().st_size > 0:
            log.info(f"  kürate[aydınlatma]: karanlık klip (YAVG {yavg:.0f}) → gamma "
                     f"{gamma} (gölge kaldırma)")
            return out_path
    except Exception as e:  # noqa: BLE001
        log.info(f"  kürate[aydınlatma]: hata ({e}) → orijinal klip")
    return None
