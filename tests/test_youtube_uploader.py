...
def test_build_status_with_publish_at_forces_private():
    s = build_status(privacy_status="public", ai_content=True,
                     publish_at="2026-06-28T06:00:00.000Z")
    # publishAt set edilir
    assert s["publishAt"] == "2026-06-28T06:00:00.000Z"
    # privacy public verilse bile private'a zorlanır (YouTube kısıtı)
    assert s["privacyStatus"] == "private"
    assert s["containsSyntheticMedia"] is True


def test_build_status_without_publish_at_unchanged():
    s = build_status(privacy_status="unlisted", ai_content=False)
