...
def test_upsert_video_stats_donusum_alanlari(tmp_path):
    # subscribers_gained + avg_view_percentage kaydedilir; verilmezse 0 (geriye uyum).
    eng = init_db(tmp_path / "x.sqlite")
    sid = record_short(eng, channel="ch", rss_item_guid=None, title="T",
                        file_path="x.mp4", duration_s=6, script_json="{}", render_ms=1)
    record_youtube_upload(eng, short_id=sid, video_id="V1",
    ...
    upsert_video_stats(
        ...
        views=100, likes=5, comments=1,
        watch_time_min=12.5, avg_view_duration_s=20.0,
        subscribers_gained=3, avg_view_percentage=187.5,
    )
    rows = get_video_stats_for_short(eng, short_id=sid, days=30)
    assert rows[0].subscribers_gained == 3
    assert rows[0].avg_view_percentage == 187.5

    # eski çağıran (yeni alanlar yok) → 0'a düşer, patlamaz
    upsert_video_stats(
        eng, video_id="V1", snapshot_date=today,
        views=150, likes=8, comments=2,
        watch_time_min=20.0, avg_view_duration_s=25.0,
    ...
    rows = get_video_stats_for_short(eng, short_id=sid, days=30)
    assert rows[0].views == 150
    assert rows[0].subscribers_gained == 0


def test_migration_eski_stats_tablosuna_donusum_kolonlari_ekler(tmp_path):
    # Üretim DB'si yeni kolonlardan önce kuruldu → init_db idempotent ALTER ile ekler.
    import sqlite3
    db = tmp_path / "old.sqlite"
    con = sqlite3.connect(db)
    con.execute("""CREATE TABLE youtube_video_stats (
        video_id TEXT NOT NULL, snapshot_date TEXT NOT NULL,
        views INTEGER, likes INTEGER, comments INTEGER,
        watch_time_min REAL, avg_view_duration_s REAL, updated_at TIMESTAMP)""")
    con.execute("""CREATE UNIQUE INDEX idx_yt_video_stats_video_date
                   ON youtube_video_stats (video_id, snapshot_date)""")
    con.commit(); con.close()

    eng = init_db(db)                     # migration burada çalışır
    sid = record_short(eng, channel="ch", rss_item_guid=None, title="T",
                        file_path="x.mp4", duration_s=6, script_json="{}", render_ms=1)
    record_youtube_upload(eng, short_id=sid, video_id="V1",
                           status="success", error=None,
                           video_url="https://youtu.be/V1")
    upsert_video_stats(
        eng, video_id="V1", snapshot_date=date.today(),
        views=1, likes=0, comments=0,
        watch_time_min=0.0, avg_view_duration_s=0.0,
        subscribers_gained=2, avg_view_percentage=45.5,
    )
    rows = get_video_stats_for_short(eng, short_id=sid, days=30)
    assert rows[0].subscribers_gained == 2
    assert rows[0].avg_view_percentage == 45.5
