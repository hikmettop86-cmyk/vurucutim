...
def test_connect_renders_html_with_google_url_when_secrets_present(tmp_path):
    """connect() should render an HTML page embedding the Google OAuth URL (not redirect)."""
...
def test_callback_exchanges_code_and_saves_token(tmp_path):
    ...
    app.config["SHORTBOT_YT_CREDS_DIR"] = yt_root
    # Pre-seed a code_verifier file so callback can find it
    pending_dir = app.config["SHORTBOT_DB_PATH"].parent / "oauth_pending"
    pending_dir.mkdir(parents=True, exist_ok=True)
    (pending_dir / "ch.json").write_text(
        json.dumps({"code_verifier": "testverifier123456789"}), encoding="utf-8"
    )
    client = app.test_client()
    ...
    # Callback now returns 200 HTML (standalone page for the external browser window)
    assert resp.status_code == 200
    body = resp.data.decode()
    assert "bağlandı" in body or "T" in body
