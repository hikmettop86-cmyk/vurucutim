...
def test_list_running_runs_excludes_runs_older_than_60_minutes(eng):
    """Live-runs panel must not show abandoned never-finished runs from > 60 min ago."""
    from datetime import datetime, timedelta, timezone
    from short_bot.db import runs as runs_table
    # Insert a 'running' row with started_at 2 hours ago via raw SQL
    old_started = datetime.now(timezone.utc) - timedelta(hours=2)
    with eng.begin() as conn:
        conn.execute(runs_table.insert().values(
            channel="ch_zombie", trigger="cron", status="running",
            started_at=old_started, log_path="z.log",
        ))
    # Insert a fresh running run
    fresh_id = start_run(eng, "ch_fresh", trigger="manual", log_path="f.log")

    from short_bot.web.activity import list_running_runs
    rows = list_running_runs(eng)
    channels = [r.channel for r in rows]
    assert "ch_fresh" in channels
    assert "ch_zombie" not in channels
