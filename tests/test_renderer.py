...
def test_build_html_includes_script_text(tmp_path):
    ...
    # like/subscribe/share (ABONE OL/BEĞEN/PAYLAŞ) etiketleri KALDIRILDI (2026-07-16,
    # kullanıcı kararı; bkz. renderer.py:16) → artık basılmaz.
    assert "ABONE OL" not in html
    assert "BEĞEN" not in html
...
def test_build_html_includes_ui_labels_when_provided(tmp_path):
    template = Path("templates/newscast.html.j2")
    # breaking etiketi override edilebilir; like/subscribe/share KALDIRILDI (2026-07-16).
    labels = {"breaking": "BREAKING NEWS", "like": "LIKE",
              "subscribe": "SUBSCRIBE", "share": "SHARE"}
    html = build_html(_job(tmp_path), template, ui_labels=labels)
    assert "BREAKING NEWS" in html         # breaking rozeti override çalışıyor
    assert "LIKE" not in html              # CTA etiketleri artık basılmıyor
...
def test_build_html_default_ui_labels_when_omitted(tmp_path):
    template = Path("templates/newscast.html.j2")
    html = build_html(_job(tmp_path), template)
    # Varsayılan TR breaking rozeti görünür; CTA etiketleri (BEĞEN/ABONE OL/PAYLAŞ) KALKTI.
    assert "SON DAKİKA" in html
    assert "ABONE OL" not in html
    assert "PAYLAŞ" not in html
