...
def test_download_and_blur_thumb_writes_blurred(tmp_path):
    ...
    with patch("short_bot.assets.requests.get") as m:
        ...
        r.content = src
        m.return_value = r
        # min_width=0 bypasses the resolution gate (added in v0.6.3 to reject
        # tiny og:images) — the fixture is a small test image.
        out = download_and_blur_thumb("http://x/thumb.jpg", cache,
                                       blur_radius=8, min_width=0)
